...
async function fetching(){
    let url = await fetch('https://raw.githubusercontent.com/Dipen-Dedania/static-data/main/india-popular-city.json');
...
async function fetching1(){
    let url1 = await fetch('https://raw.githubusercontent.com/Dipen-Dedania/static-data/main/make-your-trip-package.json');
...
async function fetching2(cityname){
    ...
    let url2 = await fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${cityname}.json?access_token=pk.eyJ1Ijoic2h1YmhhbWhpcmFuaTQ1IiwiYSI6ImNreTN3OHBiaTA2OXoyd3E5YjJ2b2xicWkifQ.hQfD_1Mmlpta37azNXVyvQ`);
    ...
    let url3 = await fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${latitude}&lon=${longitude}&exclude=hourly,daily&units=metric&appid=d57f8c3baf6bb12c1c6f23e9e1315929`);
    ...
    if(tempstatus == "Sunny"){
        condition.innerHTML = "<i class='fas fa-sun fa-2x' style='color: #eccc68'></i>";
    }
    else if(tempstatus == 'Clouds'){
        condition.innerHTML = "<i class='fas fa-cloud fa-2x' style='color: #dfe4ea'></i>";
    }
    else if(tempstatus == 'Rain'){
        condition.innerHTML = "<i class='fas fa-rain fa-2x' style='color:#a4b0be'></i>";
    }
    else{
        condition.innerHTML = "<i class='fas fa-sun fa-2x' style='color:#eccc68'></i>"
    }
